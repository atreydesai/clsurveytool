...
def main():
    ...
    # We'll modify the main pending list in place to keep non-subset entries
    # and remove processed subset entries
    
    while subset_entries:
        batch = subset_entries[:BATCH_SIZE]
        subset_entries = subset_entries[BATCH_SIZE:]
        
        print(f"\\nProcessing batch of {len(batch)} entries...")
        
        batch_processed = []
        batch_ids = set()
        
        for entry in batch:
            print(f"  Analyzing: {entry.get('title', 'Untitled')[:50]}...")
            
            notes = entry.get('analysis_notes', '')
            if not notes:
                print("    Skipping (no notes)")
                errors += 1
                continue
            ...
            # Append to subset_dataset.jsonl
            append_jsonl(SUBSET_DATASET_FILE, entry)
            batch_processed.append(entry)
        ...
        print(f"  Saved {len(batch_processed)} entries to dataset.")
        print(f"  Progress: {processed_count}/{total}")
        
        # Small delay between batches
        time.sleep(1)

    print(f"\\nComplete! Processed {processed_count} entries. Errors: {errors}")
